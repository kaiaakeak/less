...
def send_commands_to_devices(devices, filename, *, show=None, config=None, limit=3):
    with ThreadPoolExecutor(max_workers=limit) as executor:
        futures_list = [executor.submit(send_commands, dev, show=show, config=config) for dev in devices]
        result_list = [future.result() for future in as_completed(futures_list)]
...
commands_show = ['sh ip int bri', 'sh ip ospf int']
commands_config = ['route ospf 55', 'network 0.0.0.0 255.255.255.255 area 0', 'no network 0.0.0.0 255.255.255.255 area 0', 'exit', 'no route ospf 55']

if __name__ == "__main__":
    with open('devices.yaml') as f:
        devices = yaml.safe_load(f)

    pprint(send_commands_to_devices(devices, 'temp.txt', show=commands_show))